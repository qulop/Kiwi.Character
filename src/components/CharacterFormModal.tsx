...
interface CharacterFormModalProps {
  ...
  submitLabel: string;
  /** Empty values for create, or the character's current data for edit. */
  initial: NewCharacterInput;
  /** The character's own id when editing, so its current name isn't flagged. */
  excludeId?: string;
  onClose: () => void;
  onSubmit: (input: NewCharacterInput) => void | Promise<void>;
}

/**
 * Shared character form used both to create a new character and to view/edit an
 * existing one. The avatar preview renders `form.avatar` whether it's a `data:`
 * URL (freshly picked) or an `asset:` URL (existing) — <img> handles both.
 */
export default function CharacterFormModal({
  ...
  const [form, setForm] = useState<NewCharacterInput>(initial);
  ...
  const checkName = async () => {
    ...
    if (!n) return;
    try {
      setNameTaken(!(await api.characterNameAvailable(n, excludeId)));
    } catch {
      // Ignore — the submit-time check still guards against duplicates.
    }
  };
  ...
  return (
    ...
    <div className="kc-modal kc-modal--new" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="kc-modal-foot">
        ...
        <button className="kc-primary-btn kc-primary-btn--pill" disabled={!canSubmit}
          onClick={submit}>
